...
import type { ApiLeaderboardEntry } from '@/lib/api-client';
...
function LeaderboardRow({ entry, rank, type }: { entry: ApiLeaderboardEntry; rank: number; type: 'top' | 'bottom' }) {
    ...
    <section className="py-16 md:py-24 bg-white">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {[0, 1].map(col => (
            <div key={col} className="bg-slate-50 rounded-3xl p-6 border border-slate-200">
...
export default function AlignmentLeaderboardLive() {
    ...
    return (
      <section className="py-16 md:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 text-center">
          <p className="text-slate-500">Unable to load leaderboard data. Please try again later.</p>
        </div>
      </section>
    );
  }
  ...
  // Filter by chamber
  const topAligned = data!.topAligned
    .filter(entry => chamberFilter === 'all' || entry.chamber.toLowerCase() === chamberFilter)
    .slice(0, 5);
  const bottomAligned = data!.bottomAligned
    .filter(entry => chamberFilter === 'all' || entry.chamber.toLowerCase() === chamberFilter)
    .slice(0, 5);
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <div className="mt-4 flex items-center justify-center gap-6 text-sm text-slate-500">
            <span>📊 {data!.membersWithData} members analyzed</span>
            <span>•</span>
            <span>📈 Average: {data!.averageScore}% alignment</span>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="bg-gradient-to-br from-red-50 to-orange-50 rounded-3xl p-6 border border-red-200">
            ...
            <div className="space-y-2">
              {bottomAligned.map((entry, idx) => (
                <LeaderboardRow 
                  key={entry.bioguideId} 
                  entry={entry} 
                  rank={data!.membersWithData - data!.bottomAligned.length + idx + 1} 
                  type="bottom" 
                />
              ))}
            </div>
